...
const MyMemo = () => {
  ...
  const getLength = (w) => {
    console.log(`getLength(${w}) 호출 :: ${day.format("YY/MM/DD hh:mm:ss.ms")}`);
    return w.length;
  };
  ...
  // myWord를 모니터링하여 이 값이 변경되었을 때 myLen이라는 상태값을 업데이트하고자 함
  ...
  //  (1) + (2) 에 대한 통합 기능
  // 두 번째 파라미터인 배열에 설정된 state값이 이전 상태와 다를 경우에만 콜백 실행
  // 콜백의 결과가 저장되는 myLen은 일반 상태값과 동일하게 사용할 수 있다
  // 즉, myWord가 변경될 때만 콜백이 리턴하는 값을 활용하여 myLen을 갱신
