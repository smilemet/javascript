...
const EmbarkedBox = styled.span`
  &::before {
    ...
    content: "${({ embarked }) =>
      embarked === "C" ? "셰르부르" : embarked === "Q" ? "퀸즈타운" : "사우샘프턴"}";
    font-weight: 600;
  }
`;
...
const Titanic = () => {
  ...
  // --> 기본적으로 컴포넌트의 마운트와 동시에 자동 실행되어 응답 결과를 data에 저장
  ...
  // 이 컴포넌트가 화면에 마운트 되었는지 확인하는 hook
  ...
  React.useEffect(() => {
    ...
    // hook 함수 안에서 다른 상태값을 사용할 경우 해당 상태값 모니터링 필요
  ...
  if (error) {
    ...
    // 컴포넌트 자체가 함수이며, 함수가 실행 도중 리턴하므로
    // 이 내용을 화면에 표시하고 컴포넌트 실행 중단함
    return (
      <div>
