...
// arr.lastIndexOf(item, from)는 위 메서드와 동일한 기능을 하는데, 검색을 끝에서부터 시작
// 두 번째 파라미터 from이 없으면 처음부터 탐색.
...
// arr.includes(item, from)는 인덱스 from부터 시작해 item이 있는지를 검색하는데, 해당하는 요소를 발견하면 true 반환.
console.log(arr1.includes(1)); // true
console.log(arr1.includes(100)); // false


// 위 메서드들은 요소를 찾을 때 완전항동연산자(===)를 사용한다는 점에 유의
// 타입까지 같아야 함
// 요소의 위치를 정확히 알고 싶은 건 X 요소가 배열 내 존재하는지 여부만 알 경우 includes를 사용하는 게 좋다
// includes는 NaN도 제대로 처리한다는 점에서 indexOf/lastIndexOf와 약간의 차이가 있다.
const arr = [NaN];
console.log(arr.indexOf(NaN)); // -1 -> 완전항등비교는 NaN에 동작 X
...
// 주어진 판별함수를 만족하는 첫 번째 값을 반환. 못 찾으면 undefined 반환
// 찾고자 하는 항목이 아닌 검색 규칙을 구현한 콜백함수를 전달해야 함
const found = arr3.find(function(v) {
    // 파라미터로 전달되는 v는 배열의 모든 원소가 순차적으로 전달된다.
    console.log(v);

    // v를 우리가 원하는 조건에 충족하는지 검사하여 true/false를 리턴
    // true를 리턴하는 순간 배열의 검색을 중단한다. (break와 동일한 기능)
    if ( v % 2 == 0) {
...
const results = arr4.filter(function(v, i, arr) {
    ...
    if (v % 2 == 0) {
        // true가 리턴되는 경우 v는 result 배열의 원소로 저장된다
