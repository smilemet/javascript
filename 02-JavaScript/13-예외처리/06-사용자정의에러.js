...
class XlessError extends Error {
    //자식 클래스가 생성자를 가질 경우 부모의 생성자를 반드시 강제호출해야 한다. --> supper(..)
    constructor(msg) {
        super(msg);
...
class YlessError extends Error {
    // 자식 클래스가 생성자를 가질 경우 부모의 생성자 강제호출 필수
    constructor(msg) {
        super(msg);
...
function foo(x, y) {
    if (x < 0) {
        // 함수 안에서 에러를 강제로 발생시킬 경우, 이 함수를 호출하는 위치를 에러로 인식
...
// try 블록 안의 코드는 최소화하는 것이 프로그램 효율에 good
// k값을 정상적으로 리턴받았다면 그 결과값을 활용하는 처리는 try 외부에서
// 에러 점검이 끝난 후 try-catch 블록 밖에서 k값을 활용하려면
// 변수의 선언 위치가 try블록보다 상위에 위치해야 한다. --> 변수의 스코프
const a = null;
const b = null;
